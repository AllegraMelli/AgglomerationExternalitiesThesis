...
scenario_files = [
    "sobol scenario run Endgame MAR.tar.gz",
    "sobol scenario run Opposing views MAR.tar.gz",
    "sobol scenario run Storms MAR.tar.gz",
    "sobol scenario run Struggling Synergies MAR.tar.gz",
    "sobol scenario run Base MAR.tar.gz"
    ]
...
for sector in sector_names:
    ...
    for (fname, outcomes), col in zip(scenario_data, colors):
        ...
        # Highlight best run
        ax_ts.plot(time, best_run, color=col, linewidth=1.5, label=f"{fname.replace('sobol scenario run ','').replace(' MAR.tar.gz','')} peak", zorder=3)
